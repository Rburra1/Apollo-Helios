"""
Apollo-Modern ablation runner.

Runs the full 6-experiment matrix in sequence:
...
This is 7 runs, 8h each = 56h total, spread over ~7 overnights.

Use --resume to skip runs that already have a best.pt checkpoint.

Each run is its own python subprocess so there's no state bleed between
experiments and you can stop/resume cleanly.

After all runs finish, prints a results table and writes results.md.
"""
        ...
        with log_path.open() as f:
            for line in f:
                ...
                if e['val_loss'] < best_val:
                    best_val = e['val_loss']
                last_step = e['step']
                last_elapsed = e['elapsed_h']
        rows.append({
            'run': run_name,
            'preset': preset,
            'optimizer': optimizer,
            'body_M': round(cfg['param_count_body'] / 1e6, 2),
...
def write_results_md(rows, out_path):
    lines = [
        '# Apollo-Modern ablation results',
        '',
...
def main():
    ...
    ap.add_argument('--hours', type=float, default=8.0,
                    help='Wall clock per run')
    ap.add_argument('--out-dir', default='out')
    ap.add_argument('--resume', action='store_true',
                    help='Skip runs that already have a best.pt')
    ap.add_argument('--only', nargs='+', default=None,
                    help='Run only these names (e.g. --only A0_baseline A6_modern_muon)')
    ap.add_argument('--collect-only', action='store_true',
                    help='Skip running, just write results.md from existing logs')
