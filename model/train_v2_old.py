...
Everything else (LR schedule, batch sampler, eval methodology, loop order,
hyperparameters) is identical to Apollo's training script.
"""
...
def main():
    ...
    ap.add_argument('--lr', type=float, default=2.5e-4,
                    help='AdamW base LR. Muon scale uses --muon-lr.')
    ...
    # Schedule params (Apollo's exact formulas)
    ...
    # Training loop (Apollo's exact structure: eval first, then train)
    for it in range(args.iters + 1):
        # LR for this iteration
        ...
        # Throttle to target wall time (Apollo's chunked-sleep pattern)
